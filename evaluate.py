...
if __name__ == "__main__":
    ...
    # with open(os.path.join(os.path.dirname(__file__), "tasklist_tmp.yml")) as f:
    #     tasks = yaml.safe_load(f)
    ...
    for task_key in tasks:
        ...
        if task_name in results:
            ...
            results[task_name] = {
                ...
                "dataset": task_name,
                "metrics": metrics,
                "tags": tasks[task_key].get("tags", task_key)
            }
    ...
    print(f'imagenet1k : {results["ImageNet 1k"]["metrics"]["main_metric"]}')
    
    # print out the average results for imagenet
    imagenet_avg = np.mean(
        [
            val["metrics"]["main_metric"]
            for val in results.values()
            if val["metrics"]["main_metric"] is not None and
            "ImageNet" in val["dataset"] 
        ]
    )
    print(f"ImageNet Average: {imagenet_avg}")
    ...
    print(f"VTAB Average: {vtab_avg}")
    
    Retrivial_avg = np.mean(
        [
            val["metrics"]["main_metric"]
            for val in results.values()
            if val["metrics"]["main_metric"] is not None and
            "retrieval" in val["key"] or "winogavil" in val["key"]
        ]
    )
    print(f"Retrivial Average: {Retrivial_avg}")
    
    # output retrivial that dont contain winogavil
    Retrivial_avg = np.mean(
        [
            val["metrics"]["main_metric"]
            for val in results.values()
            if val["metrics"]["main_metric"] is not None and
            "retrieval" in val["key"]
        ]
    )
    print(f"Original Retrivial Average: {Retrivial_avg}")
